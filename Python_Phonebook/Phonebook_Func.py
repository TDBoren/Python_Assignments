...
def center_window(self, w, h): # pass in the tkinter frame (master) reference and the w and h
    ...
    # calculate x and y coordinates to paint the app cantered on the user's screen
...
def create_db(self):
    ...
    with conn:
        ...
        # You must commit() to save changes & close the databse connection
        conn.commit()
    conn.close()
    first_run(self)
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) >0) and (len(var_email) > 0): # enforce the user to provide both
        ...
        with conn:
            cursor = conn.cursor()
            # Check the database for existence of fullname, if so we will alert user and disregard the request
            ...
            if chkName == 0: # if this is 0 then there is no existence of the fullname and we can add new data
                ...
                messagebox.showerror("Name Error","'{}' already exists in the databese! Please choose a different name.".format(var_fullname))
...
def onDelete(self):
    ...
    with conn:
        cur=conn.cursor()
        # check count to ensure that thisi is not the last record in
...
######              onRefresh(self) # udate the listbox of the changes
                conn.commit()
        else:
...
def onUpdate(self):
    try:
        var_select = select.lstList1.curselection()[0] # index of the list selection
        var_value = select.lstList1.get(var_select) # list selection's text value
    except:
        messagebox.showinfo("Missing selection","No name was selected from the list box. \nCancelling the Udate request.")
        return
    ...
    var_phone = self.txt_phone.get().strip() # normalize the data to mnaintain database integrity
    var_email = self.txt_email.get().strip()
